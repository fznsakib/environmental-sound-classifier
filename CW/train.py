...
def main(args):
    ...
    model = CNN(height=85, width=41, channels=1, class_count=10, mode=args.mode, dropout=args.dropout, improvement_on=args.improvement)

    # MLMC input has different size so update CNN accordingly
    if args.mode == 'MLMC':
        model = CNN(height=145, width=41, channels=1, class_count=10, mode=args.mode, dropout=args.dropout, improvement_on=args.improvement)
    ...
    # In TSCNN mode, load both LMCNet and MCNet. Initialise TSCNN validator instead of generic trainer.
        ...
        # Initialise models
        lmc_model = CNN(height=85, width=41, channels=1, class_count=10, mode=args.mode, dropout=args.dropout, improvement_on=args.improvement)
        mc_model = CNN(height=85, width=41, channels=1, class_count=10, mode=args.mode, dropout=args.dropout, improvement_on=args.improvement)
